...
# ── Query Engine: traduce la pregunta a consultas SQL reales ──────────────────
...
def query_engine(db: Session, message: str) -> str:
    """
    Analiza el mensaje y ejecuta las consultas necesarias.
    Retorna un bloque de texto con los DATOS REALES de la BD
    para que Claude pueda responder con precisión.
    """
    from sqlalchemy import func, and_, or_, desc, asc
    from app.models.catalogs import (
        Project, ProjectStatus, Entity, ExecutingDepartment,
        ExecutionModality, FinancingType, OrderingOfficial
    )
    from app.models.project_modification import ProjectModification
    ...
    import re
    años = re.findall(r'\b(20\d{2})\b', message)
    año_filtro = int(años[0]) if años else None

    # ── Detectar estado mencionado ────────────────────────────────
    ESTADOS_MAP = {
        'ejecuci': 'EN EJECUCIÓN', 'activo': 'ACTIVO',
        'terminado': 'TERMINADO', 'liquidado': 'LIQUIDADO',
        'suspendido': 'SUSPENDIDO', 'formulado': 'FORMULADO',
        'suscrito': 'SUSCRITO', 'cierre': 'CIERRE FINANCIERO',
        'incorporado': 'INCORPORADO',
    }
    estado_filtro = None
    for kw, estado in ESTADOS_MAP.items():
        if kw in msg:
            estado_filtro = estado
            break

    # ── Helpers de filtro base ────────────────────────────────────
    def base_q():
        q = db.query(Project).filter(Project.is_active == True)
        if año_filtro:
            q = q.filter(Project.project_year == año_filtro)
        if estado_filtro:
            status = db.query(ProjectStatus).filter(
                func.upper(ProjectStatus.status_name).contains(estado_filtro.upper())
            ).first()
            if status:
                q = q.filter(Project.project_status_id == status.status_id)
        return q
    ...
    def enrich(p):
        """Convierte un proyecto en dict legible."""
        ...
        return {
            'id':         p.project_id,
            'año':        p.project_year,
            'externo':    p.external_project_number or f'ID-{p.project_id}',
            'nombre':     p.project_name[:100],
            'objeto':     p.project_purpose[:120] if p.project_purpose else '',
            'entidad':    entity.entity_name[:60] if entity else '—',
            'dependencia':dept.department_name[:50] if dept else '—',
            'estado':     status.status_name if status else '—',
            'valor':      float(p.project_value or 0),
            'beneficio':  float(p.institutional_benefit_value or 0),
            'aporte_UD':  float(p.university_contribution or 0),
            'inicio':     _fmt_date(p.start_date),
            'fin':        _fmt_date(p.end_date),
            'suscripcion':_fmt_date(p.subscription_date),
        }

    # ═══════════════════════════════════════════════════════════
    # 1. TOP / RANKING por valor
    # ═══════════════════════════════════════════════════════════
    if any(w in msg for w in ['mayor valor', 'más valor', 'mas valor', 'top', 'ranking',
                               'más costoso', 'mas costoso', 'más valioso', 'más caros',
                               'cuales son los proyectos', 'más alto', 'mas alto']):
        # Detectar N (top 5, top 10, etc.)
        n_match = re.search(r'top\s*(\d+)|(\d+)\s*(?:proyecto|más)', msg)
        n = int(n_match.group(1) or n_match.group(2)) if n_match else 10

        proyectos = base_q().order_by(desc(Project.project_value)).limit(n).all()
        filas = [enrich(p) for p in proyectos]
        total = base_q().count()
        valor_total = base_q().with_entities(func.sum(Project.project_value)).scalar() or 0

        ctx = f"TOP {n} PROYECTOS POR MAYOR VALOR"
        if año_filtro: ctx += f" — AÑO {año_filtro}"
        if estado_filtro: ctx += f" — ESTADO: {estado_filtro}"
        ctx += f"\nTotal proyectos en el filtro: {total} | Valor total: {_fmt_money(valor_total)}\n"
        for i, f in enumerate(filas, 1):
            ctx += f"\n{i}. {f['externo']} | {f['nombre']} | {f['entidad']} | {_fmt_money(f['valor'])} | {f['estado']} | {f['inicio']} → {f['fin']}"
        results.append(ctx)

    # ═══════════════════════════════════════════════════════════
    # 2. CONTEOS Y ESTADÍSTICAS
    # ═══════════════════════════════════════════════════════════
    if any(w in msg for w in ['cuántos', 'cuantos', 'total de', 'estadística', 'estadisticas',
                               'distribución', 'distribucion', 'resumen', 'por estado',
                               'por año', 'por entidad', 'cuánto vale', 'cuanto vale',
                               'valor total', 'suma']):
        total_proy = base_q().count()
        total_val  = base_q().with_entities(func.sum(Project.project_value)).scalar() or 0
        total_ben  = base_q().with_entities(func.sum(Project.institutional_benefit_value)).scalar() or 0

        ctx = f"ESTADÍSTICAS GENERALES"
        if año_filtro: ctx += f" — AÑO {año_filtro}"
        ctx += f"\n- Total proyectos: {total_proy}"
        ctx += f"\n- Valor total contratos: {_fmt_money(total_val)}"
        ctx += f"\n- Beneficio institucional acumulado: {_fmt_money(total_ben)}"

        # Por estado
        por_estado = (
            db.query(ProjectStatus.status_name, func.count(Project.project_id),
                     func.sum(Project.project_value))
            .join(Project, Project.project_status_id == ProjectStatus.status_id)
            .filter(Project.is_active == True)
        )
        if año_filtro:
            por_estado = por_estado.filter(Project.project_year == año_filtro)
        por_estado = por_estado.group_by(ProjectStatus.status_name).all()
        if por_estado:
            ctx += "\n\nDESGLOSE POR ESTADO:"
            for est, cnt, val in por_estado:
                ctx += f"\n  {est}: {cnt} proyectos — {_fmt_money(val)}"

        # Por año si no hay filtro de año
        if not año_filtro:
            por_año = (
                db.query(Project.project_year, func.count(Project.project_id), func.sum(Project.project_value))
                .filter(Project.is_active == True)
                .group_by(Project.project_year)
                .order_by(desc(Project.project_year))
                .limit(10).all()
            )
            if por_año:
                ctx += "\n\nDESGLOSE POR AÑO:"
                for año, cnt, val in por_año:
                    ctx += f"\n  {año}: {cnt} proyectos — {_fmt_money(val)}"

        results.append(ctx)

    # ═══════════════════════════════════════════════════════════
    # 3. MODIFICACIONES / PRÓRROGAS / ADICIONES
    # ═══════════════════════════════════════════════════════════
    if any(w in msg for w in ['prórrog', 'prorrog', 'adicion', 'adición', 'modificac',
                               'suspens', 'más modificaciones', 'mas modificaciones']):
        TIPO_MAP = {
            'prórrog': 'EXTENSION', 'prorrog': 'EXTENSION',
            'adicion': 'ADDITION', 'adición': 'ADDITION',
            'suspens': 'SUSPENSION',
        }
        tipo_mod = None
        ...
        q_mods = (
            db.query(Project.project_id, Project.external_project_number,
                     Project.project_name, Project.project_year,
                     func.count(ProjectModification.modification_id).label('n_mods'),
                     func.sum(ProjectModification.addition_value).label('total_adicion'))
            .join(ProjectModification, ProjectModification.project_id == Project.project_id)
            .filter(Project.is_active == True, ProjectModification.is_active == True)
        )
        if año_filtro:
            q_mods = q_mods.filter(Project.project_year == año_filtro)
        if tipo_mod:
            q_mods = q_mods.filter(ProjectModification.modification_type == tipo_mod)

        q_mods = (q_mods
            .group_by(Project.project_id, Project.external_project_number,
                       Project.project_name, Project.project_year)
            .order_by(desc('n_mods')).limit(15).all()
        )

        total_mods = db.query(func.count(ProjectModification.modification_id)).filter(
            ProjectModification.is_active == True).scalar()

        ctx = f"PROYECTOS CON MODIFICACIONES"
        if tipo_mod: ctx += f" (tipo: {tipo_mod})"
        if año_filtro: ctx += f" — AÑO {año_filtro}"
        ctx += f"\nTotal modificaciones en el sistema: {total_mods}\n"
        for r in q_mods:
            nombre = (r.project_name or '')[:80]
            ctx += f"\n- {r.external_project_number or f'ID-{r.project_id}'} ({r.project_year}) | {nombre} | {r.n_mods} modificaciones | Adiciones: {_fmt_money(r.total_adicion)}"
        results.append(ctx)

    # ═══════════════════════════════════════════════════════════
    # 4. POR ENTIDAD
    # ═══════════════════════════════════════════════════════════
    if any(w in msg for w in ['entidad', 'empresa', 'institución', 'institucion',
                               'contratante', 'más proyectos', 'mas proyectos']):
        # Buscar entidad específica mencionada
        # Si no hay, hacer ranking
        por_entidad = (
            db.query(Entity.entity_name,
                     func.count(Project.project_id).label('n'),
                     func.sum(Project.project_value).label('val'))
            .join(Project, Project.entity_id == Entity.entity_id)
            .filter(Project.is_active == True)
        )
        if año_filtro:
            por_entidad = por_entidad.filter(Project.project_year == año_filtro)
        por_entidad = (por_entidad
            .group_by(Entity.entity_name)
            .order_by(desc('n'))
            .limit(15).all()
        )
        ctx = f"ENTIDADES CON MÁS PROYECTOS"
        if año_filtro: ctx += f" — AÑO {año_filtro}"
        ctx += "\n"
        for ent, n, val in por_entidad:
            ctx += f"\n- {ent[:70]}: {n} proyectos — {_fmt_money(val)}"
        results.append(ctx)

    # ═══════════════════════════════════════════════════════════
    # 5. PROYECTOS ESPECÍFICOS (buscar por nombre/número)
    # ═══════════════════════════════════════════════════════════
    busqueda_keywords = ['proyecto', 'contrato', 'convenio', 'busca', 'encuentra',
                          'dame', 'muestra', 'cuál es', 'cual es', 'información de',
                          'informacion de', 'detalles de']
    if any(w in msg for w in busqueda_keywords) and not any(w in msg for w in
           ['cuántos', 'cuantos', 'total', 'ranking', 'top', 'mayor valor', 'estadística']):

        # Buscar por número externo
        num_match = re.search(r'\b([A-Z]{2,}[-_]?\d{3,}[-\w]*)\b|\b(\d{4,})\b', message.upper())
        q_search = base_q()

        if num_match:
            term = num_match.group(0)
            q_search = q_search.filter(
                or_(Project.external_project_number.ilike(f'%{term}%'),
                    Project.project_id == (int(term) if term.isdigit() else 0))
            )
        else:
            # Buscar palabras clave en nombre/objeto
            words = [w for w in message.split() if len(w) > 4
                     and w.lower() not in ['proyecto', 'contrato', 'dame', 'muestra',
                                            'cual', 'cuál', 'información', 'informacion']]
            for word in words[:3]:
                q_search = q_search.filter(
                    or_(Project.project_name.ilike(f'%{word}%'),
                        Project.project_purpose.ilike(f'%{word}%'))
                )

        proyectos = q_search.limit(10).all()
        if proyectos and len(proyectos) <= 10:
            ctx = f"PROYECTOS ENCONTRADOS ({len(proyectos)} resultado(s)):\n"
            for p in proyectos:
                e = enrich(p)
                ctx += f"\n{'='*50}"
                ctx += f"\nN°: {e['externo']} | Año: {e['año']} | Estado: {e['estado']}"
                ctx += f"\nNombre: {e['nombre']}"
                ctx += f"\nObjeto: {e['objeto']}"
                ctx += f"\nEntidad: {e['entidad']} | Dependencia: {e['dependencia']}"
                ctx += f"\nValor: {_fmt_money(e['valor'])} | Beneficio UD: {_fmt_money(e['beneficio'])}"
                ctx += f"\nFechas: {e['inicio']} → {e['fin']} | Suscripción: {e['suscripcion']}"
            results.append(ctx)

    # ═══════════════════════════════════════════════════════════
    # 6. BENEFICIO INSTITUCIONAL
    # ═══════════════════════════════════════════════════════════
    if any(w in msg for w in ['beneficio', 'aporte ud', 'aporte universidad', 'contribución',
                               'contribucion', 'universidad aporta', 'institutional']):
        q_ben = base_q()
        total_ben = q_ben.with_entities(func.sum(Project.institutional_benefit_value)).scalar() or 0
        total_ud  = q_ben.with_entities(func.sum(Project.university_contribution)).scalar() or 0
        n_conv    = base_q().filter(Project.university_contribution > 0).count()

        top_ben = (base_q()
            .filter(Project.institutional_benefit_value > 0)
            .order_by(desc(Project.institutional_benefit_value))
            .limit(10).all()
        )
        ctx = f"BENEFICIO INSTITUCIONAL SIEXUD"
        if año_filtro: ctx += f" — AÑO {año_filtro}"
        ctx += f"\n- Total beneficio acumulado: {_fmt_money(total_ben)}"
        ctx += f"\n- Total aporte UD: {_fmt_money(total_ud)}"
        ctx += f"\n- Convenios con aporte UD: {n_conv}"
        ctx += "\n\nTOP PROYECTOS POR BENEFICIO:"
        for p in top_ben:
            ctx += f"\n  {p.external_project_number or p.project_id} | {_fmt_money(p.institutional_benefit_value)} | {p.project_year}"
        results.append(ctx)

    # ═══════════════════════════════════════════════════════════
    # 7. CONTEXTO GENERAL (siempre incluir)
    # ═══════════════════════════════════════════════════════════
    total_all  = db.query(func.count(Project.project_id)).filter(Project.is_active == True).scalar()
    total_val_all = db.query(func.sum(Project.project_value)).filter(Project.is_active == True).scalar() or 0
    total_mods_all = db.query(func.count(ProjectModification.modification_id)).filter(ProjectModification.is_active == True).scalar()

    # Si no se generó contexto específico, dar resumen general
    if not results:
        por_año = (
            db.query(Project.project_year, func.count(Project.project_id), func.sum(Project.project_value))
            .filter(Project.is_active == True)
        ...
        ctx = f"RESUMEN GENERAL SIEXUD\n"
        ctx += f"- Total proyectos: {total_all} | Valor total: {_fmt_money(total_val_all)} | Modificaciones: {total_mods_all}\n"
        ctx += "POR AÑO: " + ", ".join(f"{a}:{c} ({_fmt_money(v)})" for a,c,v in por_año) + "\n"
        ctx += "POR ESTADO: " + ", ".join(f"{e}:{c}" for e,c in por_estado)
        results.append(ctx)

    general_ctx = f"\n[BD SIEXUD] Total proyectos: {total_all} | Valor total: {_fmt_money(total_val_all)} | Modificaciones: {total_mods_all}"

    return "\n\n".join(results) + general_ctx
...
async def chat_ia(data: ChatRequest, db: Session = Depends(get_db)):
    ...
    # ── 1. Detectar intención de archivo ─────────────────────────
    ...
    if intent['is_report'] and intent['file_type']:
        try:
            ...
            if ftype == 'excel':
                ...
                _buf = io.BytesIO(); wb.save(_buf); _buf.seek(0)
                content  = _buf.read()
            ...
            file_context = (
                f"ARCHIVO GENERADO: {ext.upper()} con {len(rows)} proyectos.\n"
                f"Incluye EXACTAMENTE esta línea al final de tu respuesta: [DESCARGA]:{download_url}"
            )
        except Exception as e:
            file_context = f"ERROR generando archivo: {str(e)}"

    # ── 2. Query engine: consultar BD real según la pregunta ──────
    datos_reales = query_engine(db, data.message)

    # ── 3. System prompt ──────────────────────────────────────────
    system_prompt = """Eres el Asistente IA del sistema SIEXUD — Universidad Distrital Francisco José de Caldas.
Tienes acceso COMPLETO y en TIEMPO REAL a la base de datos de proyectos de extensión universitaria.

REGLAS CRÍTICAS:
1. Responde SIEMPRE en español.
2. Usa ÚNICAMENTE los datos proporcionados en el contexto — son datos reales de la BD.
3. Si el usuario pregunta por proyectos con mayor valor, lista exactamente esos proyectos con sus valores reales.
4. Si el usuario pide una cantidad (top 5, top 10), muéstrala directamente sin preguntar.
5. Si el usuario pide una tabla, usa formato markdown con | columnas |.
6. Si el usuario pide un resumen, sé conciso pero completo con los datos reales.
7. NUNCA digas que "no tienes acceso" o que "necesitas más información" si los datos están en el contexto.
8. Si el contexto tiene datos de proyectos individuales, úsalos directamente.
9. Si hay un archivo generado con [DESCARGA]:URL, inclúyela exactamente al final.
10. Para preguntas cruzadas (modificaciones + proyectos, entidades + valores), usa todos los datos del contexto."""
    ...
    user_content = f"DATOS REALES DE LA BD SIEXUD:\n{datos_reales}\n"
    if file_context:
        user_content += f"\n{file_context}\n"
