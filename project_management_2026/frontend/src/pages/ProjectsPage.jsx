          ...
          <div style={{ padding: '10px 16px', borderTop: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--bg-hover)' }}>
            <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              {showAll
                ...
                : `Página ${cur} de ${total} · ${filtered.length} resultados`}
            </span>
            {!showAll && (
              <div style={{ display: 'flex', gap: 4 }}>
                {[
                  { icon: <ChevronsLeft size={13} />,  action: () => goTo(1),       disabled: cur === 1 },
                  { icon: <ChevronLeft  size={13} />,  action: () => goTo(cur - 1), disabled: cur === 1 },
                  { icon: <ChevronRight size={13} />,  action: () => goTo(cur + 1), disabled: cur === total },
                  { icon: <ChevronsRight size={13} />, action: () => goTo(total),   disabled: cur === total },
                ].map((b, i) => (
                  <button key={i} onClick={b.action} disabled={b.disabled} style={{
                    ...
                    opacity: b.disabled ? 0.4 : 1, fontFamily: 'inherit',
                  }}>{b.icon}</button>
                ))}
              </div>
            )}
